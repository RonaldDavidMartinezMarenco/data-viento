...
# Runs weekly to delete old weather forecasts
#
# Schedule: 0 2 * * 0 (Sunday at 2 AM)
# Log: /home/ronald/data-viento/apps/server/logs/cleanup/weather_YYYYMMDD.log
...
echo "==================================================================" >> "$LOG_FILE"
echo "Weather Cleanup - $(date)" >> "$LOG_FILE"
echo "==================================================================" >> "$LOG_FILE"

cd "$PROJECT_DIR"
# Keep 14 days of weather data (hourly + daily)
python3 -m src.tasks.cleanups.cleanup_task --days 14 >> "$LOG_FILE" 2>&1

EXIT_CODE=$?

if [ $EXIT_CODE -eq 0 ]; then
    echo "✓ Weather cleanup completed successfully" >> "$LOG_FILE"
else
    echo "✗ Weather cleanup failed with exit code $EXIT_CODE" >> "$LOG_FILE"
fi

echo "" >> "$LOG_FILE"

find "$LOG_DIR" -name "weather_*.log" -type f -mtime +90 -delete
