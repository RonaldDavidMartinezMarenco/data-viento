...
echo "==================================================================" >> "$LOG_FILE"
echo "Weather Hourly Update - $(date)" >> "$LOG_FILE"
echo "==================================================================" >> "$LOG_FILE"

cd "$PROJECT_DIR"
python3 -m src.tasks.updates.weather_update_task --hourly-only >> "$LOG_FILE" 2>&1

EXIT_CODE=$?

if [ $EXIT_CODE -eq 0 ]; then
    echo "✓ Weather hourly update completed successfully" >> "$LOG_FILE"
else
    echo "✗ Weather hourly update failed with exit code $EXIT_CODE" >> "$LOG_FILE"
fi
