...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)" #apps/server/scripts/weather
PROJECT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)" #apps/server
LOG_DIR="$PROJECT_DIR/logs/weather_update" #apps/server/logs/weather_update
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/current_$(date +%Y%m%d).log"
...
echo "==================================================================" >> "$LOG_FILE"
echo "Weather Current Update - $(date)" >> "$LOG_FILE"
echo "==================================================================" >> "$LOG_FILE"


cd "$PROJECT_DIR"
python3 -m src.tasks.updates.weather_update_task --current-only >> "$LOG_FILE" 2>&1
EXIT_CODE=$?

if [ $EXIT_CODE -eq 0 ]; then
    echo "✓ Weather current update completed successfully" >> "$LOG_FILE"
else
    echo "✗ Weather current update failed with exit code $EXIT_CODE" >> "$LOG_FILE"
fi

echo "" >> "$LOG_FILE"
find "$LOG_DIR" -name "current_*.log" -type f -mtime +7 -delete
exit $EXIT_CODE
