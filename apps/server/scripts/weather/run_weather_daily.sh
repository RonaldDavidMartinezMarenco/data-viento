...
LOG_DIR="$PROJECT_DIR/logs/weather_update"
...
echo "==================================================================" >> "$LOG_FILE"
echo "Weather Daily Update - $(date)" >> "$LOG_FILE"
echo "==================================================================" >> "$LOG_FILE"

cd "$PROJECT_DIR"
python3 -m src.tasks.updates.weather_update_task --daily-only --forecast-days 7 >> "$LOG_FILE" 2>&1

EXIT_CODE=$?

if [ $EXIT_CODE -eq 0 ]; then
    echo "✓ Weather daily update completed successfully" >> "$LOG_FILE"
else
    echo "✗ Weather daily update failed with exit code $EXIT_CODE" >> "$LOG_FILE"
fi
