...
echo "==================================================================" >> "$LOG_FILE"
echo "Marine Current Update - $(date)" >> "$LOG_FILE"
echo "==================================================================" >> "$LOG_FILE"

cd "$PROJECT_DIR"
python3 -m src.tasks.updates.marine_update_task --current-only >> "$LOG_FILE" 2>&1

EXIT_CODE=$?

if [ $EXIT_CODE -eq 0 ]; then
    echo "✓ Marine current update completed successfully" >> "$LOG_FILE"
else
    echo "✗ Marine current update failed with exit code $EXIT_CODE" >> "$LOG_FILE"
fi
