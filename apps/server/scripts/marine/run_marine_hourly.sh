...
echo "==================================================================" >> "$LOG_FILE"
echo "Marine Daily Update - $(date)" >> "$LOG_FILE"
echo "==================================================================" >> "$LOG_FILE"

cd "$PROJECT_DIR"
python3 -m src.tasks.updates.marine_update_task --daily-only --forecast-days 5 >> "$LOG_FILE" 2>&1

EXIT_CODE=$?

if [ $EXIT_CODE -eq 0 ]; then
    echo "✓ Marine daily update completed successfully" >> "$LOG_FILE"
else
    echo "✗ Marine daily update failed with exit code $EXIT_CODE" >> "$LOG_FILE"
fi
