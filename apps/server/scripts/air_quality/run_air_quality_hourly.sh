#!/bin/bash
#
# Air Quality Hourly Forecast Update Script
# Runs every 6 hours to fetch hourly forecasts
#
# Schedule: 0 */6 * * * (every 6 hours)
# Log: /home/ronald/data-viento/apps/server/logs/air_quality_update/hourly_YYYYMMDD.log

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
LOG_DIR="$PROJECT_DIR/logs/air_quality_update"
...
echo "==================================================================" >> "$LOG_FILE"
echo "Air Quality Hourly Update - $(date)" >> "$LOG_FILE"
echo "==================================================================" >> "$LOG_FILE"

cd "$PROJECT_DIR"
python3 -m src.tasks.updates.air_quality_update_task --hourly-only --forecast-days 5 >> "$LOG_FILE" 2>&1

EXIT_CODE=$?

if [ $EXIT_CODE -eq 0 ]; then
    echo "✓ Air quality hourly update completed successfully" >> "$LOG_FILE"
else
    echo "✗ Air quality hourly update failed with exit code $EXIT_CODE" >> "$LOG_FILE"
fi
